...
def get_by_title(title):
    """ Функция возвращает фильм по title """
...
def get_by_between_years(year1, year2):
    """ Функция возвращает фильмы в промежутке лет от year1 до year2 """
    db_connect = DbConnect("netflix.db")
    query = f"""
...
def get_by_rating(group_key):
    """ Функция возвращает фильмы по возрастным категориям """
    groups = {
        "children": "'G'",
        "family": "'G', 'PG', 'PG-13'",
        "adult": "'R', 'NC-17'"
...
def get_by_genre(genre):
    """ Функция возвращает фильмы по жанру """
    db_connect = DbConnect("netflix.db")
    query = f"""
...
def cast_partners(actor1, actor2):
    """ Функция возвращает актёров, которые играли с актёрами actor1 и actor2 более двух раз """
    db_connect = DbConnect("netflix.db")
    query = f"""
                    SELECT "cast"
                    FROM netflix
...
def get_movie_by_tyg(type_movie, year, genre):
    """ Функция возвращает фильмы по типу, году выпуска, жанру """
    db_connect = DbConnect("netflix.db")
    query = f"""
